...
class SineMIDISynth:
    def __init__(self, midi_input=None, *, samplerate=48000, blocksize=256, master_gain=0.2,
                 attack_ms=5, release_ms=30, backend=None, respect_sustain=True, debug=False,
                 channel=None, idle_failsafe_sec=2.0):
        """
        ...
        midi_input = mido.open_input(interfaces[1]) # TODO button to iterate over detected interfaces
        ...
        if isinstance(midi_input, mido.ports.BaseInput):
            ...
            if isinstance(midi_input, int):
                ...
                self.port_name = next((n for n in ins if midi_input.lower() in n.lower()), None)
                if self.port_name is None:
                    raise RuntimeError(f"No MIDI input matching '{midi_input}'. Found: {ins}")
            else:
    ...
    # ---------- MIDI handling ----------
    def _ch_ok(self, msg):
        return (self._channel is None) or (getattr(msg, "channel", None) == self._channel)
    ...
    def _note_on(self, n, vel):
        with self._lock:
            ...
            if st is None:
                self._notes[n] = {"phase": 0.0, "freq": self.midi_to_freq(n),
                                  "ramp": 0.0, "step": self.attack_step, "gain": g}
            else:
    ...
    def _midi_loop(self):
        ...
        for msg in self._inport:
            ...
            if msg.type == "note_on":
                if msg.velocity == 0:
                    if self._debug: print(f"[MIDI] note_off via note_on0 n={msg.note}")
                    self._note_off(msg.note)
                else:
                    if self._debug: print(f"[MIDI] note_on n={msg.note} v={msg.velocity}")
                    self._note_on(msg.note, msg.velocity)
            elif msg.type == "note_off":
                if self._debug: print(f"[MIDI] note_off n={msg.note}")
                self._note_off(msg.note)
    ...
    def _audio_cb(self, outdata, frames, time_info, status):
        ...
        if self._idle_failsafe_sec > 0:
            if (not self._sustain) and (len(self._held_keys) == 0) and (time.time() - self._last_msg_ts > self._idle_failsafe_sec):
                self._all_notes_off()
        ...
        if active:
            ...
            with self._lock:
                for n in [n for n, st in self._notes.items() if st["ramp"] <= 0.0 and st["step"] < 0]:
                    del self._notes[n]
    ...
    def start(self):
        ...
        self._audio = sd.OutputStream(channels=1, callback=self._audio_cb,
                                      samplerate=self.SR, blocksize=self.blocksize, dtype="float32")
        self._audio.start()
        if self._debug:
            print("[AUDIO] started")
    ...
    def stop(self):
        self._stop_evt.set()
        if self._audio is not None:
            self._audio.stop(); self._audio.close(); self._audio = None
        if self._listener_th is not None:
            self._listener_th.join(timeout=1.0); self._listener_th = None
        if self._owns_port and self._inport is not None:
            self._inport.close(); self._inport = None
        if self._debug:
            print("[AUDIO] stopped")
