...
def find_chord_from_abbr(abbr):
    """Resolve an abbreviation or full name to the canonical chord entry.

    Tries exact match first, then case-insensitive. Ignores extra whitespace.
    Returns a tuple (name, value) from chord_names, or None if not found.
    """
    ...
    for name, val in chord_names.items():
        if query == name or query in val["abbrvs"]:
            return name, val
    # Case-insensitive fallback
    low = query.lower()
    for name, val in chord_names.items():
        if low == name.lower() or any(low == a.lower() for a in val["abbrvs"]):
            return name, val
    return None


def is_same_chord(chord1, chord2):
    """Return True if two chord labels represent the same set of notes.

    Supports canonical names and abbreviations, including multi-word names.
    """
    if not isinstance(chord1, str) or not isinstance(chord2, str):
        return False

    # Normalize spacing
    c1 = " ".join(chord1.strip().split())
    c2 = " ".join(chord2.strip().split())

    ch1_parts = c1.split(" ")
    ch2_parts = c2.split(" ")
    if len(ch1_parts) < 2 or len(ch2_parts) < 2:
        return False

    ch1_root, ch2_root = ch1_parts[0], ch2_parts[0]
    ch1_name_or_abbr = " ".join(ch1_parts[1:])
    ch2_name_or_abbr = " ".join(ch2_parts[1:])

    info1 = find_chord_from_abbr(ch1_name_or_abbr)
    info2 = find_chord_from_abbr(ch2_name_or_abbr)
    if info1 is None or info2 is None:
        return False

    can1, can2 = info1[0], info2[0]
    notes1 = parse_chord(f"{ch1_root} {can1}")
    notes2 = parse_chord(f"{ch2_root} {can2}")
